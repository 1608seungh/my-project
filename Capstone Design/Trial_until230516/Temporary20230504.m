...
% Zero Position
M01 = eye(4); % 1st & 2nd Joint Position (1,2번 회전축이 만나는 지점을 원점으로 설정했으므로)
M02 = [eye(3), [0;0;0.1036]; 0 0 0 1]; % 3rd Joint Position
M03 = [eye(3), [0;0;0.2011]; 0 0 0 1]; % 4th Joint Position
M04 = [eye(3), [-0.015;0;0.2280]; 0 0 0 1]; % 5th Joint Position
% M05 = [eye(3), [-0.025;-0.081;0.228]; 0 0 0 1]; % 6th Joint Position
M = [eye(3), [-0.015;-0.1516;0.228]; 0 0 0 1]; 
...
q4 = [0;0;0.2011]; q5 = [-0.015;0;0.2280]; % q6 = [-0.025; -0.081; 0];
...
% Initial Angle 
i = 0;
th = [-pi/4; 0; pi/2; -pi/2; 0] % 6x1 Matrix 
...
Js = [w1,w2,w3,w4,w5; v1,v2,v3,v4,v5]; % Initial Space Jacobian Matrix; Jsi = (w_i; v_i)
...
%     x5(i) = T05(1,4); y5(i) = T05(2,4); z5(i) = T05(3,4);
    ...
    line([p_4(1), p_6(1)], [p_4(2), p_6(2)], [p_4(3), p_6(3)], 'color', 'Black', 'Linewidth', 1)
