...
%     left_plot = leftp1;
    ...
    right_pp = right_p;
    try
    left_p = interp1([yLeftY, yRightY], [xLeftY, xRightY], min(720,yLeftY),'linear','extrap');
    right_p = interp1([yLeftW, yRightW], [xLeftW, xRightW], min(720,yRightW),'linear','extrap');
    catch
        left_p = left_pp;
        right_p = right_pp;
    end
    
    center_p = (left_p+right_p)/2;
    vehicle_p = 640.5 +60;
    
    plot(center_p, 720, 'mo')
    plot(vehicle_p, 720, 'wo')
    vehicle_lat_pos(f_idx) = vehicle_p - center_p;
    f_idx = f_idx + 1;
    hold off
    drawnow
